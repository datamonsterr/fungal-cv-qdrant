...
KNOWN_ENVS = {'CREA', 'CYA', 'CYAS', 'MEA', 'DG18', 'YES', 'OA', 'MALT', 'STEFF', 'Sabouraud'}
...
def parse_filename(filename: str, species_folder: str = "") -> dict:
    ...
    name = re.sub(r'\s*(edited|detail\s*colony|Auto)\s*$', '', name, flags=re.IGNORECASE)
    ...
    if len(parts) >= 3:
        ...
        if second_last in KNOWN_ENVS:
            return {"strain": ' '.join(parts[:-2]), "environment": second_last, "angle": normalize_angle(last)}

    for env in KNOWN_ENVS:
        pattern = rf'^\S+\s+(CBS\s+\d+[_\.]\d+)\s+{env}(o|r|ob|rev|b)?$'
        match = re.match(pattern, name, re.IGNORECASE)
        if match:
            return {"strain": match.group(1), "environment": env, "angle": normalize_angle(match.group(2) or '')}
    ...
    for env in KNOWN_ENVS:
        ...
        match = re.match(pattern, name, re.IGNORECASE)
        if match:
            return {"strain": match.group(1), "environment": env, "angle": normalize_angle(match.group(2))}
        ...
        for i, p in enumerate(parts[:-2]):
            if p.lower() in {'ob', 'rev', 'o', 'r'}:
                ...
                if potential_env in KNOWN_ENVS:
                    return {"strain": ' '.join(parts[:i]), "environment": potential_env, "angle": normalize_angle(p)}
            ...
            for item in sorted(species_dir.iterdir()):
                if item.is_file():
                    ...
                    for img_file in sorted(item.iterdir()):
                        ...
                        strain = parsed["strain"] if parsed["strain"] != "UNKNOWN" else strain_name
                        images.append({
...
def draw_bboxes(image, bboxes, label="colony"):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        cv2.putText(result, f"{label}_{i}", (x, y - 5),
...
def process_pipeline(diverse_images_path: Path, metadata_path: Path, limit: int = None):
    ...
    for img_info in images:
        ...
        preprocessed = cv2.resize(cropped, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_AREA)
        ...
        if target_path.exists():
            target_filename = f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_{count}.jpg"
            target_path = env_dir / target_filename
        ...
        original_path = env_dir / f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_original.jpg"
        cv2.imwrite(str(original_path), src)
        step_paths["original"] = str(original_path.relative_to(PROJECT_ROOT))

        # Step 1: Center-cropped to square (width = height)
        centered_path = env_dir / f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_centered.jpg"
        cv2.imwrite(str(centered_path), centered)
        step_paths["centered"] = str(centered_path.relative_to(PROJECT_ROOT))

        # Step 2: Resized to 512x512 for Hough detection
        resized_path = env_dir / f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_resized.jpg"
        cv2.imwrite(str(resized_path), resized)
        step_paths["resized"] = str(resized_path.relative_to(PROJECT_ROOT))

        # Step 3: Hough-circle-cropped image
        cropped_path = env_dir / f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_cropped.jpg"
        cv2.imwrite(str(cropped_path), cropped)
        step_paths["cropped"] = str(cropped_path.relative_to(PROJECT_ROOT))

        # Step 4: Preprocessed image (final, resized to 256x256)
        cv2.imwrite(str(target_path), preprocessed)
        step_paths["preprocessed"] = str(target_path.relative_to(PROJECT_ROOT))

        # Step 4: Bboxes visualization
        bboxes_img = draw_bboxes(preprocessed, bboxes)
        bbox_path = env_dir / f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_bboxes.jpg"
        cv2.imwrite(str(bbox_path), bboxes_img)
        step_paths["bboxes"] = str(bbox_path.relative_to(PROJECT_ROOT))
        ...
        for idx, bbox in enumerate(bboxes):
            ...
            seg_path = env_dir / f"{clean_strain}_{img_info['angle']}_{img_id[:8]}_seg{idx}.jpg"
            cv2.imwrite(str(seg_path), segment_img)
            segment_paths.append(str(seg_path.relative_to(PROJECT_ROOT)))
        ...
        print(f"Processed: {img_info['species']}/{img_info['environment']}/{target_filename} -> {len(bboxes)} colonies")
        count += 1
