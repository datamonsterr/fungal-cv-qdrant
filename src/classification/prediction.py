...
from src.database.query_utils import find_nearest_neighbors_by_id, get_image_metadata, visualize_neighbors
from src.config import STRAIN_SPECIES_MAPPING_PATH, SEGMENTED_IMAGE_DIR, RESULTS_DIR

def load_strain_to_species_mapping(csv_path: str = str(STRAIN_SPECIES_MAPPING_PATH)) -> Dict[str, str]:
    """
...
def predict(
    ...
    query_images = get_all_images_for_strain(client, collection_name, strain, environment)
...
def visualize_false_predictions(
    ...
    for raw in raw_results:
        ...
        if top_neighbor.get('specy') != result['ground_truth']:
            ...
            output_path = os.path.join(output_dir, f"false_pred_{result['strain']}_{query_id}.jpg")
...
def batch_predict(
    ...
    for i, strain in enumerate(strains):
        ...
        res = predict(
            client, collection_name, strain, feature_extractor, k,
            min_samples, without_siblings, environment, "avg",
            strain_to_specy_path, segmented_image_dir, output_dir
        )
        results.append(res)
