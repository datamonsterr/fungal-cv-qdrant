...
def visualize_prediction_by_environment(
    ...
    canvas_height = header_height + num_environments * (img_height + text_height + row_spacing)
    ...
    pred_text = f"Predicted: {predicted_specy} ({confidence:.2f}) | Correct: {is_correct}"
    ...
    draw.text((padding, 60), pred_text, fill=(0, 128, 0) if is_correct else (255, 0, 0), font=title_font)
    ...
    for i, res in enumerate(aggregated_results[:5]): # Show top 5
        ...
        draw.text((legend_x + 20, legend_y), legend_text, fill=text_color, font=small_font)
    ...
    for row_idx, result in enumerate(raw_results_sorted):
        ...
        draw.text((padding, current_y - 30), f"Environment: {environment}", fill=text_color, font=title_font)
        ...
        if os.path.exists(query_path):
            try:
                ...
                draw.text((padding, current_y + img_height + 5), "Query", fill=text_color, font=text_font)
                draw.text((padding, current_y + img_height + 25), f"ID: {query_id}", fill=text_color, font=small_font)
        ...
        for i, neighbor in enumerate(neighbors):
            ...
            if os.path.exists(n_path):
                try:
                    ...
                    # Text info
                    draw.text((x_pos, current_y + img_height + 5), f"#{i+1} {n_specy}", fill=text_color, font=text_font)
                    draw.text((x_pos, current_y + img_height + 25), f"Score: {n_score:.4f}", fill=text_color, font=small_font)
                    draw.text((x_pos, current_y + img_height + 40), f"Strain: {n_strain}", fill=text_color, font=small_font)
