...
def run_predict_new(args):
    ...
    from src.feature_extraction.feature_extractors import (
        ResNet50Extractor, MobileNetV2Extractor, EfficientNetB1Extractor,
        HOGExtractor, GaborExtractor, ColorHistogramExtractor, ColorHistogramHSExtractor
    )
    from src.database.query_utils import find_nearest_neighbors_by_image
    ...
    prediction_result = {
        ...
        'predicted_specy': aggregated_results[0]['specy'] if aggregated_results else 'unknown',
        'predicted_confidence': aggregated_results[0]['score'] if aggregated_results else 0.0,
    ...
    for extractor_name, extractor in extractors:
        for strategy_name in strategies_to_run:
            for env_value in environments_to_run:
                ...
                output_dir = run_dir / f"{extractor_name.lower()}_{strategy_name}_{env_label}"
                output_dir.mkdir(exist_ok=True, parents=True)
...
def run_evaluate_all(args):
        ...
        for strategy_name, agg_strategy in all_strategies:
            for env_value, env_label in all_environments:
                ...
                try:
                    ...
                    print(f"Error reading results for {extractor_name}/{strategy_name}/{env_label}: {e}")
    
    # Generate CSV summary
    csv_path = run_dir / "accuracy_summary.csv"
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['extractor', 'strategy', 'environment', 'accuracy', 'correct', 'total', 'k']
    ...
    print(f"{'Rank':<6} {'Extractor':<18} {'Strategy':<12} {'Environment':<12} {'Accuracy':<10}")
    print("-" * 80)
    for i, result in enumerate(sorted_results[:10], 1):
        print(f"{i:<6} {result['extractor']:<18} {result['strategy']:<12} {result['environment']:<12} {result['accuracy']:.4f}")
    print("-" * 80)
...
def main():
    ...
    parser_predict_new = subparsers.add_parser("predict-new", help="Predict species for a new unseen strain from raw images")
    parser_predict_new.add_argument("--path", type=str, required=True, help="Path to strain folder (strain_name/environment_name/*.jpg)")
    parser_predict_new.add_argument("--extractor", type=str, default="resnet50", help="Feature extractor to use (resnet50, mobilenetv2, efficientnetb1, hog, gabor, colorhistogram, colorhistogramhs)")
    parser_predict_new.add_argument("--k", type=int, default=7, help="Number of neighbors")
    parser_predict_new.add_argument("--strategy", type=str, default="avg", choices=["avg", "uni"], help="Aggregation strategy")
    ...
    parser_evaluate.add_argument("--extractor", type=str, default="resnet50", help="Feature extractor to use (use 'all' to evaluate all extractors)")
    parser_evaluate.add_argument("--k", type=int, default=5, help="Number of neighbors")
    parser_evaluate.add_argument("--strategy", type=str, default="weighted", help="Aggregation strategy: all, uni (count-based), weighted/score/avg (similarity-weighted)")
    parser_evaluate.add_argument("--environment", type=str, default=None, help="Environment strategy: all, E1 (same env), E2 (all env), E3_<env> (specific env), E4_<env> (exclude env). Default: E1")
    parser_evaluate.set_defaults(func=run_evaluate)

    # Evaluate All
    parser_evaluate_all = subparsers.add_parser("evaluate-all", help="Run evaluation for all extractors, environments, and strategies with CSV summary")
    parser_evaluate_all.add_argument("--k", type=int, default=7, help="Number of neighbors")
