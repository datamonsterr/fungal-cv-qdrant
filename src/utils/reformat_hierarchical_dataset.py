...
def create_hierarchical_structure(
    ...
    try:
        ...
        for idx, bbox in enumerate(bboxes):
            ...
            segment_filename = (
                f"{clean_strain}_{environment}_{angle}_seg{idx}{FILE_EXTENSION}"
            )
...
def main():
    ...
    if not os.path.exists(STRAIN_SPECIES_MAPPING_PATH):
        print(
            f"Error: Strain-to-species mapping not found at {STRAIN_SPECIES_MAPPING_PATH}"
        )
        return
    ...
    if os.path.exists(HIERARCHICAL_DATASET_PATH):
        response = input(
            f"{HIERARCHICAL_DATASET_PATH} already exists. Remove and recreate? (y/n): "
        )
        if response.lower() == "y":
    ...
    for dir_name in sorted(os.listdir(ORIGINAL_DATASET_PATH)):
        ...
        for filename in sorted(os.listdir(dir_path)):
            ...
            if segments_saved > 0:
                ...
                images_per_dir[dir_key] = (
                    images_per_dir.get(dir_key, 0) + segments_saved
                )
            else:
    ...
    with open(stats_file, "w") as f:
        json.dump(
            {"summary": stats, "images_per_directory": images_per_dir}, f, indent=2
        )
