...
def compare_strategies():
    ...
    # Load both results
    weighted_results = load_ensemble_results(f"{RESULTS_DIR}/ensemble_results_weighted.json")
    simple_results = load_ensemble_results(f"{RESULTS_DIR}/ensemble_results_simple_avg.json")
    
    # Find differences
    differences = []
    for w_pred, s_pred in zip(weighted_results['ensemble_predictions'], 
                              simple_results['ensemble_predictions']):
    ...
    for i, diff in enumerate(differences, 1):
        ...
        for model, pred in diff['individual'].items():
            ...
            print(f"    {model:20s}: {pred['predicted']:35s} (conf: {pred['confidence']:.4f}) {status}")
        
        print("\n  Weighted Ensemble:")
        w_status = "✓" if diff['weighted']['correct'] else "✗"
        print(f"    Predicted: {diff['weighted']['predicted']} (conf: {diff['weighted']['confidence']:.4f}) {w_status}")
        print(f"    Top 3:")
        ...
        print(f"    Predicted: {diff['simple']['predicted']} (conf: {diff['simple']['confidence']:.4f}) {s_status}")
        print(f"    Top 3:")
    ...
    weighted_better = sum(1 for d in differences 
                         if d['weighted']['correct'] and not d['simple']['correct'])
    simple_better = sum(1 for d in differences 
                       if d['simple']['correct'] and not d['weighted']['correct'])
    both_correct = sum(1 for d in differences 
                      if d['weighted']['correct'] and d['simple']['correct'])
    both_wrong = sum(1 for d in differences 
