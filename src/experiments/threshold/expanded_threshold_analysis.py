...
def generate_formulas(scores: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    weight_sets = {
        ...
        "log_dcy": [1.0, 1/math.log(2), 1/math.log(3), 1/math.log(4), 1/math.log(5)],
        "sqrt_dcy": [1.0, 1/math.sqrt(2), 1/math.sqrt(3), 1/math.sqrt(4), 1/math.sqrt(5)],
    ...
    for alpha in [0.3, 0.5, 0.7]:
        ...
        formulas[f"hyb_gg_a{int(a*10)}"] = (
            a * (s[:, 0] - s[:, 1]) + (1 - a) * np.sqrt(s[:, 0] * s[:, 1] + eps)
        )
    ...
    # ----- Normalised rank -----
    for k in [3, 5]:
        formulas[f"norm_rnk{k}"] = (s[:, 0] - s[:, k-1]) / (s[:, 0] + s[:, k-1] + eps)
...
def run():
    ...
    print(f"  Precision={b['prec']:.4f}  Recall={b['rec']:.4f}  Specificity={b['spec']:.4f}")
    ...
    for i, r in enumerate(ranked[:20], 1):
        print(f"  {i:2d}. {r['formula']:30s} F1={float(r['f1']):.4f}  "
