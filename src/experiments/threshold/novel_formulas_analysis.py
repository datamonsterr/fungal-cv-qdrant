...
def generate_formulas(scores: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    formulas["prod_rat01_12"] = (s[:, 0] / (s[:, 1] + eps)) * (s[:, 1] / (s[:, 2] + eps))
    formulas["prod_rat12_23"] = (s[:, 1] / (s[:, 2] + eps)) * (s[:, 2] / (s[:, 3] + eps))
    formulas["prod_rat02_13"] = (s[:, 0] / (s[:, 2] + eps)) * (s[:, 1] / (s[:, 3] + eps))
    formulas["prod_rat01_23"] = (s[:, 0] / (s[:, 1] + eps)) * (s[:, 1] / (s[:, 2] + eps))
    formulas["prod_rat01_34"] = (s[:, 0] / (s[:, 1] + eps)) * (s[:, 2] / (s[:, 3] + eps))
    formulas["prod_rat02_23"] = (s[:, 0] / (s[:, 2] + eps)) * (s[:, 1] / (s[:, 3] + eps))

    # ── 2. Three-term ratio sums ─────────────────────────────────────────────
    formulas["rat012_sum"] = s[:, 0]/(s[:, 1]+eps) + s[:, 1]/(s[:, 2]+eps) + s[:, 2]/(s[:, 3]+eps)
    formulas["rat123_sum"] = s[:, 1]/(s[:, 2]+eps) + s[:, 2]/(s[:, 3]+eps) + s[:, 3]/(s[:, 4]+eps)
    formulas["rat013_sum"] = s[:, 0]/(s[:, 1]+eps) + s[:, 1]/(s[:, 3]+eps) + s[:, 2]/(s[:, 4]+eps)
    formulas["rat012_13_sum"] = s[:, 0]/(s[:, 1]+eps) + s[:, 1]/(s[:, 2]+eps) + s[:, 0]/(s[:, 3]+eps)
    ...
    formulas["rat12_cbrt"] = np.power(np.clip(s[:, 1]/(s[:, 2]+eps), 0, None), 1/3)
    ...
    formulas["poly2_01_12_sum"] = (s[:, 0]**2 - s[:, 1]**2) + (s[:, 1]**2 - s[:, 2]**2)
    formulas["poly2_01_12_prod"] = (s[:, 0]**2 - s[:, 1]**2) * (s[:, 1]**2 - s[:, 2]**2)
    ...
    formulas["min_ov_max12"] = np.minimum(s[:, 0], s[:, 1]) / (np.maximum(s[:, 0], s[:, 1]) + eps)
    ...
    formulas["log_rat01_p_log_rat12"] = (
        np.log1p(np.clip(s[:, 0]/(s[:, 1]+eps), 0, None)) +
    ...
    formulas["rat03_prod_rat14"] = (s[:, 0]/(s[:, 3]+eps)) * (s[:, 1]/(s[:, 4]+eps))
    formulas["rat01_prod_rat34"] = (s[:, 0]/(s[:, 1]+eps)) * (s[:, 2]/(s[:, 3]+eps))

    # ── 11. Score spread / diversity metrics ─────────────────────────────────
    formulas["spread_012"] = s[:, :3].std(axis=1) / (s[:, :3].mean(axis=1) + eps)
    formulas["spread_012_ov_s0"] = (s[:, :3].std(axis=1) / (s[:, :3].mean(axis=1) + eps)) / (s[:, 0] + eps)
    formulas["range_ov_s0"] = (s[:, :3].max(axis=1) - s[:, :3].min(axis=1)) / (s[:, 0] + eps)
    formulas["iqr_12"] = (np.percentile(s[:, :3], 75, axis=1) - np.percentile(s[:, :3], 25, axis=1)) / (s[:, 0] + eps)

    # ── 12. Harmonic-fraction variants ──────────────────────────────────────
    formulas["hm_rat01_12"] = 2 / (1/(s[:, 0]/(s[:, 1]+eps)+eps) + 1/(s[:, 1]/(s[:, 2]+eps)+eps) + eps)
    formulas["hm_gap01_gap12"] = 2 / (1/(s[:, 0]-s[:, 1]+eps) + 1/(s[:, 1]-s[:, 2]+eps) + eps)
    ...
    formulas["rat010_112_prod"] = (s[:, 0]/(s[:, 1]+eps)) * (s[:, 0]/(s[:, 2]+eps)) * (s[:, 1]/(s[:, 2]+eps))
    formulas["rat01_12_23_prod"] = (s[:, 0]/(s[:, 1]+eps)) * (s[:, 1]/(s[:, 2]+eps)) * (s[:, 2]/(s[:, 3]+eps))
    formulas["rat010_213_prod"] = (s[:, 0]/(s[:, 1]+eps)) * (s[:, 0]/(s[:, 3]+eps)) * (s[:, 1]/(s[:, 3]+eps))
    ...
    formulas["rat01_p_rat12_p_rat23"] = (
        s[:, 0]/(s[:, 1]+eps) + s[:, 1]/(s[:, 2]+eps) + s[:, 2]/(s[:, 3]+eps)
    )
    formulas["rat01_p_rat12_m_rat23"] = (
        s[:, 0]/(s[:, 1]+eps) + s[:, 1]/(s[:, 2]+eps) - s[:, 2]/(s[:, 3]+eps)
    )
    formulas["rat01_x_rat12_p_rat23"] = (
        (s[:, 0]/(s[:, 1]+eps)) * (s[:, 1]/(s[:, 2]+eps)) + s[:, 2]/(s[:, 3]+eps)
    )
    formulas["rat01_p_rat12_x_rat23"] = (
        s[:, 0]/(s[:, 1]+eps) + (s[:, 1]/(s[:, 2]+eps)) * (s[:, 2]/(s[:, 3]+eps))
    )
    ...
    formulas["inv_gnorm01_p_inv_gnorm12"] = (
        (s[:, 0]+s[:, 1]+eps)/(s[:, 0]-s[:, 1]+eps) +
    ...
    formulas["s0_ov_hm12"] = s[:, 0] / (2/(1/(s[:, 0]+eps)+1/(s[:, 1]+eps)+eps) + eps)
    ...
    formulas["gap01_ov_gap01_p_gap12"] = (s[:, 0]-s[:, 1]) / ((s[:, 0]-s[:, 1])+(s[:, 1]-s[:, 2])+eps)
    formulas["gap01_p_gap12_ov_gap02"] = ((s[:, 0]-s[:, 1])+(s[:, 1]-s[:, 2])) / (s[:, 0]-s[:, 2]+eps)
...
def _update_best(best, fname, algo, f1, t, m):
    if float(f1) > best["f1"]:
        best.update({
            "strategy": fname, "algorithm": algo, "f1": float(f1),
            "threshold": float(t), "precision": float(m["precision"]),
            "recall": float(m["recall"]), "specificity": float(m["specificity"]),
            "tp": int(m["tp"]), "fp": int(m["fp"]),
            "tn": int(m["tn"]), "fn": int(m["fn"]),
        })
...
def run():
    ...
    for fname in formula_names:
        ...
        all_results.append({**row_base, "algorithm": "f1_grid", "threshold": f"{t:.6f}",
                             "f1": f"{m['f1']:.6f}", "precision": f"{m['precision']:.6f}",
                             "recall": f"{m['recall']:.6f}", "specificity": f"{m['specificity']:.6f}",
                             "tp": m["tp"], "fp": m["fp"], "tn": m["tn"], "fn": m["fn"], "auroc": ""})
        ...
        try:
            ...
            all_results.append({**row_base, "algorithm": "roc_opt", "threshold": f"{t_roc:.6f}",
                                 "f1": f"{m_roc['f1']:.6f}", "precision": f"{m_roc['precision']:.6f}",
                                 "recall": f"{m_roc['recall']:.6f}", "specificity": f"{m_roc['specificity']:.6f}",
                                 "tp": m_roc["tp"], "fp": m_roc["fp"], "tn": m_roc["tn"], "fn": m_roc["fn"],
                                 "auroc": f"{auroc:.6f}"})
        ...
        all_results.append({**row_base, "algorithm": "otsu", "threshold": f"{t_otsu:.6f}",
                             "f1": f"{m_otsu['f1']:.6f}", "precision": f"{m_otsu['precision']:.6f}",
                             "recall": f"{m_otsu['recall']:.6f}", "specificity": f"{m_otsu['specificity']:.6f}",
                             "tp": m_otsu["tp"], "fp": m_otsu["fp"], "tn": m_otsu["tn"], "fn": m_otsu["fn"], "auroc": ""})
        _update_best(best_overall, fname, "otsu", m_otsu["f1"], t_otsu, m_otsu)

    # Save all results
    fields = ["formula", "algorithm", "threshold", "f1", "precision", "recall",
    ...
    for algo in ALGORITHMS:
        ...
        print(f"  {algo:12s}: F1={float(best['f1']):.4f} ({best['formula']})  "
              f"[t={float(best['threshold']):.4f}]")

    b = best_overall
    print(f"\n=== Overall best ===")
    print(f"  {b['strategy']}_{b['algorithm']}: F1={b['f1']:.4f}")
    print(f"  Precision={b['precision']:.4f}  Recall={b['recall']:.4f}  Specificity={b['specificity']:.4f}")
    ...
    for i, r in enumerate(ranked[:20], 1):
        print(f"  {i:2d}. {r['formula']:35s} F1={float(r['f1']):.4f}  "
    ...
    with open(best_path, "w") as f:
        json.dump({
            "strategy": b["strategy"], "algorithm": b["algorithm"],
            "f1": b["f1"], "threshold": b["threshold"],
