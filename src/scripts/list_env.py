...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"OVERALL ENVIRONMENT DISTRIBUTION")
    print(f"{'='*80}")
    ...
    print(f"\nEnvironment Coverage:")
    strains_with_all_envs = sum(1 for env_dict in strain_env_counts.values() 
                                 if len(env_dict) == len(env_counts))
    print(f"  Strains with samples in all environments: {strains_with_all_envs}")
    
    species_with_all_envs = sum(1 for env_dict in species_env_counts.values() 
                                 if len(env_dict) == len(env_counts))
