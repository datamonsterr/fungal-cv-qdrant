...
def inspect_model_architecture(weights_path: str, model_name: str):
    ...
    try:
        ...
        for i, layer in enumerate(model.layers):
            ...
            print(f"Layer {i}: {layer.name:<40} {layer.__class__.__name__:<30} Output: {output_shape}")
        ...
        for i, layer in enumerate(model.layers):
            if 'global_average_pooling' in layer.name.lower():
                gap_layer = layer
                gap_index = i
                print(f"✓ Found at Layer {i}: {layer.name} (Output shape: {layer.output_shape})")
                break
        
        if gap_layer is None:
            print("⚠️  GlobalAveragePooling2D layer not found by name, trying index 2...")
            if len(model.layers) > 2:
                gap_layer = model.layers[2]
                gap_index = 2
                print(f"Using Layer 2: {gap_layer.name} (Output shape: {gap_layer.output_shape})")
...
def compare_feature_extraction():
    ...
    print(f"  Feature range: [{features_imagenet.min():.4f}, {features_imagenet.max():.4f}]")
    ...
    if os.path.exists(RESNET50_WEIGHTS):
        ...
        print(f"  Feature range: [{features_finetuned.min():.4f}, {features_finetuned.max():.4f}]")
        ...
        if cosine_sim > 0.95:
            ...
            print("✓ Features are different, fine-tuned weights are being used correctly")
        print()
    else:
...
def main():
    ...
    print()
    print("1. ✅ FIXED: Feature extractors now correctly extract from GlobalAveragePooling layer")
    print()
    print("2. 🔄 RE-EXTRACT FEATURES: You MUST re-extract features with fixed extractors:")
