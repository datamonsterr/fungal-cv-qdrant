...
def load_complementary_cases():
    ...
    cases_file = "./results/ensemble_analysis/complementary_cases/complementary_cases_list.json"
...
def visualize_strain_with_extractor(
    ...
    if result_idx >= len(test_sets) or result_idx < 0:
        print(f"    ✗ Test set {test_set_index} not found (only {len(test_sets)} test sets available)")
        return None
    
    test_group = test_sets[result_idx]
    
    print(f"  Running prediction for test set {test_set_index} with {extractor_name}...")
...
def main():
    ...
    if resnet_only:
        print(f"[1/3] Generating {len(resnet_only)} ResNet50 correction visualizations...")
        print("-"*80)
        for idx, case in enumerate(resnet_only, 1):
            print(f"Case {idx}/{len(resnet_only)}: {case['strain']} test_set_{case['test_set_index']}")
            total_count += 1
            try:
    ...
    if efficient_only:
        print(f"[2/3] Generating {len(efficient_only)} EfficientNetV2B0 correction visualizations...")
        print("-"*80)
        for idx, case in enumerate(efficient_only, 1):
            print(f"Case {idx}/{len(efficient_only)}: {case['strain']} test_set_{case['test_set_index']}")
            total_count += 1
            try:
                result = visualize_strain_with_extractor(
                    client=CLIENT,
    ...
    print(f"[3/3] Generating {len(all_colorhist_wrong)} ColorHistogramHS wrong visualizations...")
    print("-"*80)
    for idx, case in enumerate(all_colorhist_wrong, 1):
        print(f"Case {idx}/{len(all_colorhist_wrong)}: {case['strain']} test_set_{case['test_set_index']}")
        total_count += 1
        try:
            result = visualize_strain_with_extractor(
                client=CLIENT,
