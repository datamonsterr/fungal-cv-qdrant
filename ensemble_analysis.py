...
def load_results_json(file_path: str) -> EvaluationResults:
    ...
    print(f"    Metadata: {results.metadata.feature_extractor}, {results.metadata.eval_strategy}, {results.metadata.aggregation_strategy}")
    return results
...
def load_accuracy_from_csv(csv_path: str) -> Dict[str, AccuracyData]:
    ...
    for row in reader:
        ...
        key = (data.feature_extractor, data.environment_strategy, data.aggregation_strategy)
        accuracy_dict[key] = data
    
    return accuracy_dict


def find_result_file(base_dir: str, feature_extractor: str, env_strategy: str, agg_strategy: str) -> Optional[str]:
    """
    ...
    json_files = [f for f in os.listdir(folder_path) if f.endswith('.json') and f.startswith('results_')]
...
def analyze_false_cases(
    ...
    def create_lookup(results: EvaluationResults) -> Dict[Tuple[str, int], PredictionResult]:
...
def get_ground_truth_rank(aggregated_results: List[AggregatedResult], ground_truth: str) -> Optional[int]:
    """
...
def visualize_correction_analysis(analysis: Dict[str, Any], output_dir: str):
    ...
    bars = axes[0].bar(categories, values, color=colors, edgecolor='black', linewidth=1.5)
    axes[0].set_ylabel('Number of Cases', fontsize=12)
    axes[0].set_title('Cases Where ColorHistogramHS is Wrong\nvs Other Models Correct', fontsize=14, fontweight='bold')
    ...
    correctable = (len(analysis['colorhist_wrong_resnet_correct']) + 
                  len(analysis['colorhist_wrong_efficient_correct']) +
                  len(analysis['colorhist_wrong_both_correct']))
    
    improvement_data = {
        'ColorHist Alone': analysis['total_predictions'] - total_wrong,
        'Potential with\nEnsemble': analysis['total_predictions'] - total_wrong + correctable,
    }
    
    bars2 = axes[1].bar(improvement_data.keys(), improvement_data.values(), 
                       color=['#ff6b6b', '#95e1d3'], edgecolor='black', linewidth=1.5)
    axes[1].set_ylabel('Correct Predictions', fontsize=12)
    axes[1].set_title('Potential Improvement with Ensemble', fontsize=14, fontweight='bold')
    ...
    axes[1].axhline(y=analysis['total_predictions'], color='gray', 
                   linestyle='--', linewidth=2, alpha=0.7, label='Total Predictions')
    axes[1].legend()
...
def create_strategy_comparison_chart(
    ...
    strategies = ['Weighted\nSum', 'Simple\nAverage']
    accuracies = [
        weighted_result['accuracy'] * 100,
        simple_result['accuracy'] * 100
    ]
    ...
    for fe in weighted_result['feature_extractors']:
        individual_correct = sum(
            1 for p in weighted_result['ensemble_predictions']
    ...
    bars = axes[0, 0].bar(strategies, accuracies, color=colors_chart1, 
    ...
    if manual_result:
        ...
        all_three_correct = sum(1 for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds)
                               if wp['correct'] and sp['correct'] and mp['correct'])
        manual_only = sum(1 for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds)
                         if not wp['correct'] and not sp['correct'] and mp['correct'])
        weighted_only = sum(1 for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds)
                           if wp['correct'] and not sp['correct'] and not mp['correct'])
        simple_only = sum(1 for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds)
                         if not wp['correct'] and sp['correct'] and not mp['correct'])
        two_correct = sum(1 for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds)
                         if (wp['correct'] + sp['correct'] + mp['correct']) == 2)
        all_three_wrong = sum(1 for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds)
                             if not wp['correct'] and not sp['correct'] and not mp['correct'])
        
        categories = ['All 3\nCorrect', '2/3\nCorrect', 'Manual\nOnly', 'Weighted\nOnly', 'Simple\nOnly', 'All 3\nWrong']
        values = [all_three_correct, two_correct, manual_only, weighted_only, simple_only, all_three_wrong]
        colors_chart2 = ['#00b894', '#95e1d3', '#a29bfe', '#4ecdc4', '#45b7d1', '#ff6b6b']
    else:
        # Two-way comparison (original)
        both_correct = sum(1 for wp, sp in zip(weighted_preds, simple_preds)
                          if wp['correct'] and sp['correct'])
        weighted_only = sum(1 for wp, sp in zip(weighted_preds, simple_preds)
                           if wp['correct'] and not sp['correct'])
        simple_only = sum(1 for wp, sp in zip(weighted_preds, simple_preds)
                         if not wp['correct'] and sp['correct'])
        both_wrong = sum(1 for wp, sp in zip(weighted_preds, simple_preds)
    ...
    bars2 = axes[0, 1].bar(categories, values, color=colors_chart2,
    ...
    if manual_result:
        ...
        bp = axes[1, 0].boxplot([weighted_confidences, simple_confidences, manual_confidences],
        ...
        axes[1, 0].text(1, weighted_mean, f'μ={weighted_mean:.3f}', 
                       ha='left', va='center', fontsize=9, fontweight='bold')
        axes[1, 0].text(2, simple_mean, f'μ={simple_mean:.3f}', 
                       ha='left', va='center', fontsize=9, fontweight='bold')
        axes[1, 0].text(3, manual_mean, f'μ={manual_mean:.3f}', 
                       ha='left', va='center', fontsize=9, fontweight='bold')
    else:
        bp = axes[1, 0].boxplot([weighted_confidences, simple_confidences],
    ...
    if manual_result:
        ...
        for wp, sp, mp in zip(weighted_preds, simple_preds, manual_preds):
            preds = [wp['predicted_specy'], sp['predicted_specy'], mp['predicted_specy']]
        ...
        if differences:
            ...
            all_wrong = sum(1 for d in differences 
                           if not d['weighted_correct'] and not d['simple_correct'] and not d['manual_correct'])
            weighted_best = sum(1 for d in differences 
                               if d['weighted_correct'] and not d['simple_correct'] and not d['manual_correct'])
            simple_best = sum(1 for d in differences 
                             if not d['weighted_correct'] and d['simple_correct'] and not d['manual_correct'])
            manual_best = sum(1 for d in differences 
                             if not d['weighted_correct'] and not d['simple_correct'] and d['manual_correct'])
            multiple_correct = sum(1 for d in differences 
                                  if (d['weighted_correct'] + d['simple_correct'] + d['manual_correct']) >= 2)
            
            diff_categories = ['All\nWrong', 'Weighted\nBest', 'Simple\nBest', 'Manual\nBest', '2+\nCorrect']
            diff_values = [all_wrong, weighted_best, simple_best, manual_best, multiple_correct]
            colors_chart4 = ['#ff6b6b', '#4ecdc4', '#45b7d1', '#a29bfe', '#00b894']
            
            bars4 = axes[1, 1].bar(diff_categories, diff_values, color=colors_chart4,
                                  edgecolor='black', linewidth=1.5, alpha=0.8)
            axes[1, 1].set_ylabel('Number of Cases', fontsize=12, fontweight='bold')
            axes[1, 1].set_title(f'Analysis of {len(differences)} Different Predictions', 
                               fontsize=14, fontweight='bold')
            axes[1, 1].set_ylim(0, max(diff_values) * 1.3 if max(diff_values) > 0 else 1)
            ...
            axes[1, 1].text(0.5, 0.5, 'All strategies predict the same',
                           ha='center', va='center', transform=axes[1, 1].transAxes,
                           fontsize=14, fontweight='bold')
            axes[1, 1].set_title('Analysis of Different Predictions', 
                               fontsize=14, fontweight='bold')
    else:
        # Two-way comparison (original)
        differences = []
        ...
        if differences:
            ...
            both_wrong_diff = sum(1 for d in differences 
                                 if not d['weighted_correct'] and not d['simple_correct'])
            weighted_better_diff = sum(1 for d in differences 
                                      if d['weighted_correct'] and not d['simple_correct'])
            simple_better_diff = sum(1 for d in differences 
                                   if not d['weighted_correct'] and d['simple_correct'])
            both_correct_diff = sum(1 for d in differences 
                                  if d['weighted_correct'] and d['simple_correct'])
            
            diff_values = [both_wrong_diff, weighted_better_diff, simple_better_diff, both_correct_diff]
            ...
            axes[1, 1].text(0.5, 0.5, 'No differences in predictions',
                           ha='center', va='center', transform=axes[1, 1].transAxes,
                           fontsize=14, fontweight='bold')
            axes[1, 1].set_title('Analysis of Different Predictions', 
                               fontsize=14, fontweight='bold')
    ...
    for idx, (model_name, model_key) in enumerate(models):
        ...
        for case in analysis['all_colorhist_wrong']:
            if model_key == 'colorhist':
                ...
                key = (case.strain, case.test_set_index)
        ...
        if model_key == 'colorhist':
            ...
            cases_to_check = []
            for case_dict in (analysis['colorhist_wrong_resnet_correct'] +
        ...
        axes[idx].axvline(mean_rank, color='red', linestyle='--', linewidth=2, 
                         label=f'Mean: {mean_rank:.1f}')
        axes[idx].axvline(median_rank, color='green', linestyle='--', linewidth=2,
                         label=f'Median: {median_rank:.1f}')
        axes[idx].legend()
        
        # Add text box with stats
        stats_text = f'n = {len(ranks)}\nMean: {mean_rank:.2f}\nMedian: {median_rank:.1f}'
...
def combine_aggregated_results(
    ...
    if debug:
        print(f"  [DEBUG combine_aggregated_results]")
        ...
        if strategy == 'manual_weighted' and species_weights:
            print(f"    Using manual species weights: {len(species_weights.weights)} species")
    ...
    for pred, weight in predictions:
        for agg_result in pred.aggregated_results:
            if strategy == 'weighted_sum':
                ...
                if species_weights:
                    ...
                    if species_name in species_weights.weights:
                        model_weight = species_weights.weights[species_name].get(pred.feature_extractor, 1.0)
                    else:
                        # Use default weights
                        model_weight = species_weights.weights.get('default', {}).get(pred.feature_extractor, 1.0)
                    
                    contribution = agg_result.score * model_weight
                    species_scores[agg_result.specy] += contribution
                    
                    if debug and agg_result.specy in ['Penicillium aurantiogriseum', 'Penicillium viridicatum', 'Penicillium cyclopium', 'Penicillium freii']:
                        print(f"    Model {pred.feature_extractor}: {agg_result.specy} score={agg_result.score:.4f}, manual_weight={model_weight:.4f}, contribution={contribution:.4f}")
                else:
            ...
            if debug and strategy != 'manual_weighted' and agg_result.specy in ['Penicillium aurantiogriseum', 'Penicillium viridicatum']:
                print(f"    Model {pred.feature_extractor}: {agg_result.specy} score={agg_result.score:.4f}, weight={weight:.4f}, contribution={contribution:.4f}")
...
def evaluate_ensemble(
    ...
    for fe in feature_extractors:
        ...
        if key in accuracy_dict:
            ...
            print(f"Warning: No accuracy data found for {fe}, {env_strategy}, {agg_strategy}")
            weights[fe] = 0.0
    ...
    for key, preds in predictions_by_key.items():
        ...
        if debug_first:
            ...
            for fe in feature_extractors:
                ...
                print(f"    Aggregated results (top 3):")
        ...
        ensemble_predictions.append({
            ...
            'individual_predictions': {
                fe: {'predicted': preds[fe].predicted_specy, 
...
def save_ensemble_results(ensemble_result: Dict[str, Any], output_dir: str, suffix: str = ''):
...
def print_ensemble_summary(ensemble_result: Dict[str, Any]):
    ...
    print("\n" + "="*80)
    print(f"ENSEMBLE EVALUATION RESULTS ({ensemble_result.get('combination_strategy', 'weighted_sum').upper()})")
    print("="*80)
    ...
    print(f"Combination Strategy: {ensemble_result.get('combination_strategy', 'weighted_sum')}")
    print(f"\nAccuracy: {ensemble_result['accuracy']*100:.2f}%")
    print(f"Correct: {ensemble_result['correct_count']}/{ensemble_result['total_predictions']}")
    ...
    for fe in ensemble_result['feature_extractors']:
        ...
        individual_correct = sum(
            1 for p in ensemble_result['ensemble_predictions']
            if p['individual_predictions'][fe]['correct']
        )
...
def visualize_complementary_cases(
    ...
    if resnet_cases:
        ...
        for idx, case in enumerate(resnet_cases, 1):
            ...
            try:
                result = regenerate_prediction_with_details(strain, test_idx, 'ResNet50', resnet_meta)
                output_path = os.path.join(resnet_output_dir, f"{idx:03d}_{strain.replace(' ', '_')}_correct.jpg")
    ...
    if efficient_cases:
        ...
        for idx, case in enumerate(efficient_cases, 1):
            ...
            try:
                result = regenerate_prediction_with_details(strain, test_idx, 'EfficientNetV2B0', efficient_meta)
                output_path = os.path.join(efficient_output_dir, f"{idx:03d}_{strain.replace(' ', '_')}_correct.jpg")
                visualize_prediction_by_environment(
                    prediction_result=result,
                    segmented_image_dir=SEGMENTED_IMAGE_DIR,
                    output_path=output_path,
    ...
    for idx, pred in enumerate(all_wrong, 1):
        ...
        try:
            result = regenerate_prediction_with_details(strain, test_idx, 'ColorHistogramHS', colorhist_meta)
            output_path = os.path.join(wrong_output_dir, f"{idx:03d}_{strain.replace(' ', '_')}_false.jpg")
            visualize_prediction_by_environment(
                prediction_result=result,
                segmented_image_dir=SEGMENTED_IMAGE_DIR,
                output_path=output_path,
    ...
    if resnet_cases:
        ...
        print(f"    ({len(resnet_cases)} cases where ResNet50 correct, ColorHistogramHS wrong)")
    if efficient_cases:
        print(f"  - {os.path.join(viz_output_dir, 'efficient_only')}/")
        print(f"    ({len(efficient_cases)} cases where EfficientNetV2B0 correct, ColorHistogramHS wrong)")
    print(f"  - {os.path.join(viz_output_dir, 'wrong_colorhistogramhs')}/")
    print(f"    ({len(all_wrong)} cases where ColorHistogramHS wrong)")
    
    if all_wrong:
        correction_rate = (len(resnet_cases) + len(efficient_cases)) / len(all_wrong) * 100
...
def main():
    ...
    CSV_PATH = os.path.join(BASE_DIR, "comprehensive_evaluation_summary_20251211_135108.csv")
    ...
    for fe, results in results_dict.items():
        ...
        if num_agg > 0:
            ...
            print(f"    [ERROR] No aggregated results found!")
    ...
    print(f"  - ResNet50 correct only: {len(analysis['colorhist_wrong_resnet_correct'])}")
    print(f"  - EfficientNetV2B0 correct only: {len(analysis['colorhist_wrong_efficient_correct'])}")
    ...
    complementary_cases_path = os.path.join(complementary_cases_dir, "complementary_cases_list.json")
    ...
    if os.path.exists(SPECIES_WEIGHTS_PATH):
        ...
        print(f"  Skipping manual weighted strategy")
    ...
    if ensemble_result_manual:
        save_ensemble_results(ensemble_result_manual, OUTPUT_DIR, suffix='_manual_weighted')
    ...
    print(f"Simple Average Ensemble: {ensemble_result_simple['accuracy']*100:.2f}% ({ensemble_result_simple['correct_count']}/{ensemble_result_simple['total_predictions']})")
    if ensemble_result_manual:
        print(f"Manual Weighted Ensemble: {ensemble_result_manual['accuracy']*100:.2f}% ({ensemble_result_manual['correct_count']}/{ensemble_result_manual['total_predictions']})")
    print(f"Best Individual Model:   {max(ensemble_result_weighted['weights'].values())*100:.2f}% (ColorHistogramHS)")
    ...
    for i, (wp, sp) in enumerate(zip(ensemble_result_weighted['ensemble_predictions'], 
    ...
    print("or use example_visualize_prediction.py with specific strain/test_set combinations.")
    print()
    print(f"Found {len(analysis['colorhist_wrong_resnet_correct'])} ResNet50 corrections")
    print(f"Found {len(analysis['colorhist_wrong_efficient_correct'])} EfficientNetV2B0 corrections")
    print(f"Found {len(analysis['all_colorhist_wrong'])} ColorHistogramHS wrong cases total")
    ...
    print(f"  - ground_truth_rankings.png")
    print(f"  - strategy_comparison.png")
    print(f"  - ensemble_results_weighted.json")
    print(f"  - ensemble_results_simple_avg.json")
    if ensemble_result_manual:
        print(f"  - ensemble_results_manual_weighted.json")
    print(f"  - complementary_cases/ (detailed visualizations)")
    print(f"    * resnet_only/ (cases where ResNet50 correct, ColorHistogramHS wrong)")
    print(f"    * efficient_only/ (cases where EfficientNetV2B0 correct, ColorHistogramHS wrong)")
    print(f"    * wrong_colorhistogramhs/ (all ColorHistogramHS wrong cases)")
