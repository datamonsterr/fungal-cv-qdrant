...
def create_detailed_comparison_chart(results, output_path):
    ...
    model_names = feature_extractors + ['Ensemble']
    
    # Shorten model names for display
    display_names = [fe.replace('ColorHistogramHS', 'ColorHist')
                            .replace('EfficientNetV2B0', 'Efficient')
                     for fe in feature_extractors] + ['Ensemble']
    
    for pred in predictions:
        correct_vector = [pred['individual_predictions'][fe]['correct'] 
                         for fe in feature_extractors] + [pred['correct']]
    ...
    sns.heatmap(agreement_matrix, annot=True, fmt='.1f', cmap='YlGnBu',
                ...
                ax=axes[0, 0], cbar_kws={'label': 'Agreement %'})
    axes[0, 0].set_title('Model Agreement Matrix\n(% of cases both correct)', 
                         fontsize=12, fontweight='bold')
    
    # Chart 2: Individual vs Ensemble performance by species
    species_stats = defaultdict(lambda: {'ensemble': 0, 'total': 0, **{fe: 0 for fe in feature_extractors}})
    ...
    width = 0.8 / num_bars
    colors = ['#ff6b6b', '#4ecdc4', '#45b7d1', '#f38181', '#aa96da', '#fcbad3', '#ffffd2']
    
    for i, fe in enumerate(feature_extractors):
        offset = (i - num_bars/2 + 0.5) * width
        short_name = fe.replace('ColorHistogramHS', 'ColorHist').replace('EfficientNetV2B0', 'Efficient')
        axes[0, 1].bar(x + offset, accuracies[fe], width, 
                      label=short_name, color=colors[i % len(colors)], alpha=0.8)
    
    # Ensemble bar (always last)
    offset = (num_bars - num_bars/2 - 0.5) * width
    axes[0, 1].bar(x + offset, accuracies['Ensemble'], width, 
                   label='Ensemble', color='#95e1d3', alpha=0.8, edgecolor='black', linewidth=2)
    ...
    confidences = {}
    for fe in feature_extractors:
        confidences[fe] = [p['individual_predictions'][fe]['confidence'] for p in predictions]
    confidences['Ensemble'] = [p['predicted_confidence'] for p in predictions]
    
    # Prepare data for boxplot
    confidence_data = [confidences[fe] for fe in feature_extractors] + [confidences['Ensemble']]
    labels = [fe.replace('ColorHistogramHS', 'ColorHist').replace('EfficientNetV2B0', 'Efficient') 
              for fe in feature_extractors] + ['Ensemble']
    
    axes[1, 0].boxplot(confidence_data,
                       labels=labels,
                       patch_artist=True,
    ...
    for test_idx in test_set_indices:
        ...
        for fe in feature_extractors:
            cumulative_correct[fe].append(
                sum(1 for p in test_predictions 
    ...
    for i, fe in enumerate(feature_extractors):
        short_name = fe.replace('ColorHistogramHS', 'ColorHist').replace('EfficientNetV2B0', 'Efficient')
        axes[1, 1].plot(test_set_indices, cumulative_correct[fe], 
                       marker=markers[i % len(markers)], label=short_name, 
                       linewidth=2, color=colors[i % len(colors)])
    
    # Ensemble line (always last, prominent)
    axes[1, 1].plot(test_set_indices, cumulative_correct['Ensemble'], 
                   marker='D', label='Ensemble', linewidth=3, color='#95e1d3', 
...
def print_detailed_report(results):
    ...
    for fe in feature_extractors:
        correct = sum(1 for p in results['ensemble_predictions']
                     if p['individual_predictions'][fe]['correct'])
        print(f"{fe:<{max_name_len}}: {correct}/{total} ({correct/total*100:.2f}%)")
    
    ensemble_correct = results['correct_count']
    print(f"{'Ensemble':<{max_name_len}}: {ensemble_correct}/{total} ({ensemble_correct/total*100:.2f}%)")
    ...
    print(f"Cases where ensemble beats all:         {len(improvements['ensemble_better_than_all'])}")
