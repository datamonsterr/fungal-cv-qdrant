    ...
    for key, colorhist_pred in colorhist_lookup.items():
        if not colorhist_pred['correct']:
            ...
            if resnet_pred and resnet_pred['correct']:
                ...
                print(f"✓ ResNet50 corrects ColorHistogramHS: {key[0]} test_set_{key[1]}")
                print(f"  Ground truth: {colorhist_pred['ground_truth']}")
                print(f"  ColorHistogramHS predicted: {colorhist_pred['predicted_specy']} (wrong)")
                print(f"  ResNet50 predicted: {resnet_pred['predicted_specy']} (correct)")
            ...
            if efficient_pred and efficient_pred['correct']:
                ...
                print(f"✓ EfficientNetV2B0 corrects ColorHistogramHS: {key[0]} test_set_{key[1]}")
                print(f"  Ground truth: {colorhist_pred['ground_truth']}")
                print(f"  ColorHistogramHS predicted: {colorhist_pred['predicted_specy']} (wrong)")
                print(f"  EfficientNetV2B0 predicted: {efficient_pred['predicted_specy']} (correct)")
...
def regenerate_prediction_with_details(
    ...
    """
    from config import QDRANT_COLLECTION_FEATURES, QDRANT_HOST, QDRANT_PORT
...
def main():
    ...
    print(f"  Cases where ResNet50 correct (ColorHistogramHS wrong): {len(resnet_only)}")
    print(f"  Cases where EfficientNetV2B0 correct (ColorHistogramHS wrong): {len(efficient_only)}")
    print()
    
    if len(resnet_only) == 0 and len(efficient_only) == 0:
        print("⚠ No complementary cases found!")
        print("This explains why ensemble doesn't improve - models make the same mistakes.")
        return
    ...
    if resnet_only:
        ...
        for idx, (strain, test_idx, ground_truth, model) in enumerate(resnet_only, 1):
            print(f"  [{idx}/{len(resnet_only)}] Regenerating prediction for {strain} test_set_{test_idx}...")
            try:
    ...
    if efficient_only:
        ...
        for idx, (strain, test_idx, ground_truth, model) in enumerate(efficient_only, 1):
            print(f"  [{idx}/{len(efficient_only)}] Regenerating prediction for {strain} test_set_{test_idx}...")
            try:
    ...
    for idx, (strain, test_idx, ground_truth, model) in enumerate(all_wrong, 1):
        print(f"  [{idx}/{len(all_wrong)}] Regenerating prediction for {strain} test_set_{test_idx}...")
        try:
    ...
    if resnet_only:
        ...
        print(f"    ({len(resnet_only)} cases where ResNet50 correct, ColorHistogramHS wrong)")
    if efficient_only:
        print(f"  - {os.path.join(output_base_dir, 'efficient_only')}/")
        print(f"    ({len(efficient_only)} cases where EfficientNetV2B0 correct, ColorHistogramHS wrong)")
    ...
    # Analysis insight
    if all_wrong:
        correction_rate = (len(resnet_only) + len(efficient_only)) / len(all_wrong) * 100
        print(f"Correction rate: {correction_rate:.1f}% of ColorHistogramHS errors were corrected by other models")
        print(f"This low rate explains why ensemble (66.67%) performs worse than ColorHistogramHS alone (75%)")
    print()
