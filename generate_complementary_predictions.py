...
from config import SEGMENTED_IMAGE_DIR, QDRANT_COLLECTION_FEATURES
import pandas as pd
...
def find_complementary_cases(
    ...
    for key, colorhist_pred in colorhist_lookup.items():
        if not colorhist_pred['correct']:
            ...
            all_wrong_cases.append((strain, test_set_idx, ground_truth, 'ColorHistogramHS'))
            
            # Check if ResNet50 is correct
            resnet_pred = resnet_lookup.get(key)
            if resnet_pred and resnet_pred['correct']:
                resnet_only_cases.append((strain, test_set_idx, ground_truth, 'ResNet50'))
            ...
            if efficient_pred and efficient_pred['correct']:
                efficient_only_cases.append((strain, test_set_idx, ground_truth, 'EfficientNetV2B0'))
...
def regenerate_prediction_with_details(
    ...
    """
    print(f"  Regenerating prediction for {strain} test_set_{test_set_index} with {feature_extractor}...")
    ...
    test_images = strain_data[strain_data['test_set'] == test_set_index]['image_id'].tolist()
...
def main():
    ...
    if resnet_only:
        ...
        print(f"  ✓ Saved {len(resnet_detailed_predictions)} predictions to {output_file}")
        print()
    
    # 2. EfficientNetV2B0 corrections
    if efficient_only:
        print(f"[2/3] Generating {len(efficient_only)} EfficientNetV2B0 correction predictions...")
        ...
        print(f"  ✓ Saved {len(efficient_detailed_predictions)} predictions to {output_file}")
        print()
